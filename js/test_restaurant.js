...
console.log(app.isOpened(9, 24));



let orderPerson = {
	personName: "Հասմիկ",
	personSurname: "Կարապետյան",
	wantToGetOrder: true,
	payMethod: "PayPal",
	isOpenToGetOrder: app.restaurantDetails[7].isOpen,
	canPersonGetOrder() {
		if (orderPerson.isOpenToGetOrder) {
	...
	whoIsCook(cook) {
		if (orderPerson.isOpenToGetOrder) {
			return cook === "Արտակ Ադամյան"
				? "ես ունեմ պատվեր"
				: "Կներեք,ես պատվեր չունեմ";
		} else {
			return `Կներեք մենք փակ ենք`;
		}
	},
	getDrink(drink) {
		if (orderPerson.isOpenToGetOrder) {
			if (Object.values(app.restaurantMenu[2]).includes(drink)) {
				return "Այո մենք ունենք ձեր ուզած խմիչքը";
			} else {
				return `Մենյուն հետեւյալն է ${Object.values(app.restaurantMenu[2])}`;
			}
		} else {
			return `Կներեք մենք փակ ենք`;
		}
...
console.log(orderPerson.canPersonGetOrder());